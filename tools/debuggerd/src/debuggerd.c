...
static int find_and_open_tombstone(void)
{
    ...
    unsigned long mtime = 0xffffffff;
    struct stat sb;
    char path[128];
    int fd, i, oldest = 0;
...
static void wait_for_user_action(unsigned tid, struct ucred* cr)
{
    ...
    LOGD("debuggerd resuming process %d", cr->pid);
#endif
...
static void handle_crashing_process(int fd)
{
	...
	printf("handle_crashing_process01  (%d)\n", fd);
...
printf("BOOM: pid=%d uid=%d gid=%d tid=%d\n", cr.pid, cr.uid, cr.gid, tid);
	...
	printf("handle_crashing_process02\n");
    ...
    close(fd);
    fd = -1;
printf("handle_crashing_process03\n");
    ...
    for(;;) {
		...
		printf("handle_crashing_process04\n");
            ...
            case SIGSTKFLT: {
				...
				printf("handle_crashing_process05\n");
...
printf("debuggerd: " __DATE__ " " __TIME__ "\n");
    for(;;) {
